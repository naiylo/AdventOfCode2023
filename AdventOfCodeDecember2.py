...
example2 = """Game 1: 2 green, 12 blue; 6 red, 6 blue; 8 blue, 5 green, 5 red; 5 green, 13 blue; 3 green, 7 red, 10 blue; 13 blue, 8 red
...
def calibrateSum(string):
  ...
  for i in range(0,len(string)):
    if string[i] == "\n" or i == (len(string) - 1):
      line = string[startIndex:i+1]
      startIndex = i+1
      correctGame = 0
      #print("currentGame: " + str(currentGame))
      for j in range(0,len("Game XXX:")):
        if line[j] == ":":
          numbers = line[j+2:len(line)]
          startIndexOfRotation = 0
          for k in range(0,len(numbers)):
            if numbers[k] == ";" or k == (len(numbers) - 1):
              rotation = numbers[startIndexOfRotation:k]
              #print("Rotation: " + rotation)
              startIndexOfRotation = k+2
              startIndexOfPick = 0
              for l in range(0,len(rotation)): 
                if rotation[l] == ",":
                  pick = rotation[startIndexOfPick:l]
                  #print("Pick: " + pick)
                  startIndexOfPick = l+2
                  for m in range(0,len(pick)):
                    if pick[m] == " ":
                      number = pick[:m]
                      colour = pick[m+1]
                      #print("Number: " + number + " Colour: " + colour + " Limit: " + limits[colour])
                      if int(number) > int(limits[colour]):
                        correctGame = 1
                if l == (len(rotation)-1):
                  pick = rotation[startIndexOfPick:]
                  #print("Pick: " + pick)
                  for m in range(0,len(pick)):
                    if pick[m] == " ":
                      number = pick[:m]
                      ...
                      if int(number) > int(limits[colour]):
                        correctGame = 1
      #print("correctGame: " + str(correctGame))
      if correctGame == 0:
        sum += currentGame
      currentGame += 1
  return sum
...
def calibrateViewest(string):
  ...
  for i in range(0,len(string)):
    if string[i] == "\n" or i == (len(string) - 1):
      ...
      minimalGreen = 1
      minimalBlue = 1
      #print("Current game: " + str(currentGame))
      for j in range(0,len("Game XXX:")):
        if line[j] == ":":
          numbers = line[j+2:len(line)]
          startIndexOfRotation = 0
                ...
                if rotation[l] == ",":
                  ...
                  for m in range(0,len(pick)):
                    if pick[m] == " ":
                      ...
                      colour = pick[m+1]
                      if colour == "r":
                        if int(number) > minimalRed:
                          minimalRed = int(number)
                      if colour == "b":
                        if int(number) > minimalBlue:
                          minimalBlue = int(number)
                      if colour == "g":
...
if __name__ == "__main__":
  ...
  print("Result with the Function calibrateViewest:")
  print("Example 1:")
  print(calibrateViewest(example1))
  print("Example 2:")
  print(calibrateViewest(example2))