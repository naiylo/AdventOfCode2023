...
example3 = """F-.FJ7|777FF|-J7-|7.L.7-77--.FJ7F|J.FLJJ7.77F-7--L7-.F-FFL-JJ-JFF-|F|FF7.FF-F.JJ-LF.|.77|7F--L7FL--FFJFF-77F-7FFF-7LJ7F7---L-7-F7J.F|.7F7-77
...
def findNext(cur, x, y, direction):
                if cur == "7":
                    if direction == 0:
                        return [[x-1, y], 1]
                    return [[x, y+1], 3]
...
def findFurthestPipe(string):
    ...
    #                        y                     x
    possibleStarts = [[rowNumberOfStart+1,columnNumberOfStart,3], 
                      [rowNumberOfStart-1,columnNumberOfStart,0],
                      [rowNumberOfStart,columnNumberOfStart+1,2],
                      [rowNumberOfStart,columnNumberOfStart-1,1]]
    ...
    for i in possibleStarts:
        ...
        try: 
            while aktPipe != "S":
                ...
                next = findNext(aktPipe, aktX, aktY, aktDirection)
                aktX = next[0][0]
                aktY = next[0][1]
                # print("next:")
                aktDirection = next[1]
    ...
    mostfrequent = max(counterList, key=counterList.count)
    ...
    for i in possibleStarts:
        ...
        try: 
            while aktPipe != "S":
                ...
                loop.append([aktY, aktX])
            counterList.append(counter)
            loops.append([loop, counter])
        except: pass
    mostfrequent = max(counterList, key=counterList.count)
    # Mark the Polygon to check if points are inside or not
    for i in loops:
         if i[1] == mostfrequent:
              polygon = i[0]
              break
...
if __name__ == "__main__":
    ...
    print("Example 3:")
    print(findFurthestPipe(example3))
    print("Return with function calculateEnclosed:")
    print("Example 4:")
    print(calculateEnclosed(example4))
    print("Example 3:")
    print(calculateEnclosed(example3))
    print("Example 5:")
    print(calculateEnclosed(example5))