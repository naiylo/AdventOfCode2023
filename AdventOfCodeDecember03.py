...
def calibrateSum(string):
  ...
  while i < len(string):
    ...
    if string[i].isdigit():
      endIndex = i
      while string[endIndex].isdigit():
        endIndex += 1
      skips = endIndex - i 
      number = string[i:endIndex] 
      for j in range(i-1,endIndex+1):
        try:
          if string[j+length+1] != "." and string[j+length+1].isdigit() == False and string[j+length+1] != "\n" and (j+length+1 > 0):
            correct = 1
        except: pass
        try: 
          if string[j-length-1] != "." and string[j-length-1].isdigit() == False and string[j-length-1] != "\n" and (j-length-1 > 0):  
        ...
        try: 
          if string[i-1]  != "." and string[i-1].isdigit() == False and string[i-1] != "\n":
            correct = 1
        except: pass
        try:
          if string[endIndex]  != "." and string[endIndex].isdigit() == False and string[endIndex] != "\n":
            correct = 1
        except: pass
      i += skips
      if correct == 1:
